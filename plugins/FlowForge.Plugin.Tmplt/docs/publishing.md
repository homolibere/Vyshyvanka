...
```bash
dotnet pack src/FlowForge.Plugin.YourName.csproj -c Release -p:PackageVersion=1.2.0
```
...
3. Create a key with glob pattern `FlowForge.Plugin.YourName` and Push scope
...
dotnet nuget push src/bin/Release/FlowForge.Plugin.YourName.1.0.0.nupkg \
...
FlowForge.Core is excluded from the package (`ExcludeAssets=runtime`) because the FlowForge host provides it at runtime.

...
/// Feature: vyshyvanka, Property 9: Execution State Persistence
/// </summary>
public class ExecutionPersistenceTests : IDisposable
{
    ...
    private static VyshyvankaDbContext CreateFreshDbContext()
    {
        var options = new DbContextOptionsBuilder<VyshyvankaDbContext>()
            .UseInMemoryDatabase(databaseName: Guid.NewGuid().ToString())
            .Options;

        var dbContext = new VyshyvankaDbContext(options);
    ...
    /// <summary>
    /// Feature: vyshyvanka, Property 19: Execution History Filtering

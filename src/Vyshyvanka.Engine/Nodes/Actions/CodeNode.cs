...
/// <summary>
/// An action node that executes user-provided JavaScript code using the Jint interpreter.
/// Jint is a pure .NET JavaScript engine with no access to the host filesystem, network,
/// or .NET reflection — providing a secure sandbox by design.
/// Supports two execution modes: run once for all input, or run once per item.
/// </summary>
[NodeDefinition(
    Name = "Code",
    Description = "Execute custom JavaScript code to transform data, implement logic, or perform calculations",
    Icon = "fa-solid fa-code")]
[NodeInput("input", DisplayName = "Input", IsRequired = false)]
[NodeOutput("output", DisplayName = "Output")]
[ConfigurationProperty("code", "code",
    Description = "JavaScript code to execute. Return a value to pass it to the output.",
    IsRequired = true)]
...
public class CodeNode : BaseActionNode
{
    ...
    public override Task<NodeOutput> ExecuteAsync(NodeInput input, IExecutionContext context)
    {
        try
        {
            ...
            var result = ExecuteJavaScript(code, mode, input, context, timeoutCts.Token);

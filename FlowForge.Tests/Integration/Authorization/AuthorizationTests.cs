...
public class AuthorizationTests : IClassFixture<FlowForgeApiFixture>, IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        _viewerClient = await _fixture.CreateAuthenticatedClientAsync(
            FlowForgeApiFixture.TestUsers.Viewer.Email,
            FlowForgeApiFixture.TestUsers.Viewer.Password);

        _editorClient = await _fixture.CreateAuthenticatedClientAsync(
            FlowForgeApiFixture.TestUsers.Editor.Email,
            FlowForgeApiFixture.TestUsers.Editor.Password);
    }
    ...
    public async Task WhenAuthenticatedClientCallsPermittedEndpointsThenReceives2xxResponses()
    {
        ...
        var testCases = new List<(TestUser User, string Endpoint, HttpMethod Method, Func<HttpClient, Task<HttpResponseMessage>> Request)>
        {
            ...
            (FlowForgeApiFixture.TestUsers.Viewer, "/api/workflow", HttpMethod.Get, 
                client => client.GetAsync("/api/workflow")),
            
            // Editor can view workflows (GET)
            (FlowForgeApiFixture.TestUsers.Editor, "/api/workflow", HttpMethod.Get, 
                client => client.GetAsync("/api/workflow")),
            
            // Admin can view workflows (GET)
            (FlowForgeApiFixture.TestUsers.Admin, "/api/workflow", HttpMethod.Get, 
                client => client.GetAsync("/api/workflow")),
            
            // Editor can create workflows (POST)
            (FlowForgeApiFixture.TestUsers.Editor, "/api/workflow", HttpMethod.Post, 
            ...
            (FlowForgeApiFixture.TestUsers.Admin, "/api/workflow", HttpMethod.Post, 
                client => client.PostAsJsonAsync("/api/workflow", TestDataFactory.CreateValidWorkflow($"PropertyTest_{Guid.NewGuid():N}"))),
        };
    ...
    {
        var secretKey = "FlowForgeDefaultSecretKey123456789012345678901234567890";
